...
export function FastSlowFigure(){
 ...
 return <figure className="learning-figure"><FigureTitle title="A temporary association joins a stable model" text="The slow network still performs its learned transform. A fast outer-product memory stores Sector K → LUMEN, then adds one temporary correction to the output scores."/>
  <div className="memory-system" role="img" aria-label="Stable token model with a fast associative memory"><div className="memory-column"><small>cue</small><b>Sector K</b><span>encoded as x</span></div>
   ...
   </div><div className="control-pair"><Range label="Plasticity η" value={plasticity} min="0" max="1" step=".01" display={plasticity.toFixed(2)} set={setPlasticity}/><Range label="Delay after learning" value={delay} min="0" max="12" step="1" display={delay+" steps"} set={setDelay}/></div><ForgettingCurve plasticity={plasticity} delay={delay}/>
  </figure>
}
function ForgettingCurve({plasticity,delay}){const W=720,H=300,x=d=>54+d/12*620,y=v=>24+(1-v)*220,points=Array.from({length:13},(_,d)=>{const t=plasticity*Math.exp(-d/5);return {d,lumen:clamp(.17+.76*t,0,1),ember:.42*(1-t),north:.28*(1-t)}}),series=[["lumen","LUMEN"],["ember","EMBER"],["north","NORTH"]],path=k=>points.map((p,i)=>(i?"L":"M")+x(p.d)+","+y(p[k])).join(" "),current=points[delay],winner=series.reduce((a,b)=>current[b[0]]>current[a[0]]?b:a,series[0]);return <figure className="forgetting-curve"><figcaption><b>Recall changes as the fast trace fades</b><span>The stored answer begins with a temporary advantage. Once that trace becomes too weak, the frozen model’s original preference returns.</span></figcaption><svg viewBox={"0 0 "+W+" "+H} role="img" aria-label={"Forgetting curves at delay "+delay+", where "+winner[1]+" has the highest score"}>{[0,.25,.5,.75,1].map(v=><g key={v}><line className="grid" x1="54" y1={y(v)} x2="674" y2={y(v)}/><text x="45" y={y(v)+4} textAnchor="end">{Math.round(v*100)}%</text></g>)}<line className="current-delay" x1={x(delay)} y1="24" x2={x(delay)} y2={y(0)}/>{series.map(([k,label],i)=><g key={k}><path className={"forget-series s"+i} d={path(k)}/><circle className={"forget-point s"+i} cx={x(delay)} cy={y(current[k])} r="5"/><text className={"series-label s"+i} x="682" y={y(points[12][k])+4}>{label}</text></g>)}<text className="delay-label" x={Math.min(x(delay)+7,610)} y="40">step {delay}</text><text className="axis-title" x="364" y="288" textAnchor="middle">steps since the Hebbian write</text></svg><p>At step {delay}, <b>{winner[1]}</b> leads. The LUMEN trace is {Math.round(plasticity*Math.exp(-delay/5)*100)}% of its initial scale.</p></figure>}
